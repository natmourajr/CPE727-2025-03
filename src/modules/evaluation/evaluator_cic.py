...
from src.dataloaders.CIC_MalMem.tabular_loader import MalMemTabularDataset # Usando o DataLoader que criamos
from src.dataloaders.CIC_MalMem.image_loader import MalMemImageDataset # Usando o DataLoader que criamos
# ----------------------------------------------------------------------
...
def plot_roc_curve(y_true, y_probs, fold_name, set_name, save_dir):
    ...
    if num_classes > 2:
        # Não tentamos plotar ROC para Multiclasse, apenas calculamos o AUC OVR
...
def find_optimal_threshold(y_true, y_probs):
    """
    Encontra o limiar de decisão timo usando o método de Youden's J.
    (Só deve ser chamado para problemas binários).
    """
...
def run_kfold_evaluation(model_class, model_name, config, X_data_path, Y_data_path, dataset_class, experiment_dir, criterion=None, early_stopper_class=None, pretrain_weights_path=None):
    ...
    for fold, (train_relative_indices, val_relative_indices) in enumerate(kf.split(X_dev, Y_dev)):
        ...
        model = model_class(input_shape=input_shape_arg, num_classes=num_classes, config=config, pretrain_weights_path=pretrain_weights_path).to(device)
        ...
        if len(np.unique(val_y_true)) > 1:
            ...
            fold_metrics['auc_validation_geral'] = float(auc_val_geral) # <--- AQUI ESTAVA FALTANDO A ATRIBUIÇÃO UNIVERSAL
    ...
    final_model = model_class(input_shape=input_shape_arg, num_classes=num_classes, config=config, pretrain_weights_path=pretrain_weights_path ).to(device)
