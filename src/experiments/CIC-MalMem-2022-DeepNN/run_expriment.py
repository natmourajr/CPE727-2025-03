...
def load_config():
    """Carrega o arquivo de configura��o config.yaml."""
    config_path = PROJECT_ROOT / 'config.yaml'
    try:
        with open(config_path, 'r') as file:
            return yaml.safe_load(file)
    except FileNotFoundError:
        print(f"ERRO: Arquivo de configura��o n�o encontrado em: {config_path}")
        sys.exit(1)
        
def main():
    config = load_config()
    model_name = "DeepNN_MLP" 
    
    # --- SETUP DO EXPERIMENTO ---
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    experiment_dir = os.path.join(PROJECT_ROOT, 'results', model_name, timestamp)
    os.makedirs(experiment_dir, exist_ok=True)
    print(f"Artefatos deste experimento ser�o salvos em: {experiment_dir}")
    shutil.copy(PROJECT_ROOT / 'config.yaml', os.path.join(experiment_dir, 'config.yaml'))
    
    # 1. Definir os caminhos dos dados
    X_PATH = PROCESSED_DATA_DIR / 'X_tabular.npy' 
