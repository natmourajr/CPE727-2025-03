...
def slice_data(data_path, file_list):
    ...
    if len(os.listdir(sliced_data_path)) < (19 * len(files)):
        ...
        for record_name in files:
            ...
            """
            sensor_shape, events = tonic.io.read_dvs_128(
                data_path + "recordings/" + record_name + ".aedat"
            )
            # read the tag file
            tagfile = pd.read_csv(
                data_path + "tags/" + record_name + ".csv"
            )  # df
            ...
            sliced_events = tonic.slicers.slice_events_at_indices(
                events, events_start, events_end
            )
            # save 19 separate events on disk
            for i, chosen_slice in enumerate(sliced_events):
                np.save(
                    sliced_data_path
                    + "{}_{}.npy".format(record_name, str(i).zfill(2)),
                    chosen_slice,
                )
...
def animate_events(
    dataset, sample_index, time_window=50, frame_rate=24, repeat=False
):
    """
    ...
    frame_transform = transforms.Compose(
        [
            ...
            transforms.ToFrame(
                sensor_size, time_window=time_window  # bin into frames
            ),  # in [ms] # type: ignore
        ]
    )
...
def plot_events(dataset, sample_index):
    ...
    frame_transform = transforms.Compose(
        [
            ...
            transforms.TimeAlignment(),  # 1st event at t=0
            transforms.ToFrame(
                sensor_size, n_time_bins=3
            ),  # events -> 3 frames  # type: ignore
        ]
    )
    frames = frame_transform(sample_events)
...
def kfold_split(fileList, seed, export_txt=False):
    ...
    def gen():
        ...
        for i, (train_index, test_index) in enumerate(
            kf.split(files), start=1
        ):
            train_set, test_set = files[train_index], files[test_index]
            if export_txt:
                np.savetxt(
                    "../data/trainlist{}.txt".format(i), train_set, fmt="%s"
                )
                np.savetxt(
                    "../data/testlist{}.txt".format(i), test_set, fmt="%s"
                )
...
def get_optimizer(net, params):
    if params["optimizer"] == "adam":
        ...
        opt = optim.Adagrad(
            net.get_trainable_parameters(), lr=params["learning_rate"]
        )
    else:
        raise ValueError(
            "Please provide a valid entry for optimizer. Possible choices are: adam, adamax, adagrad"
        )
    return opt
...
def get_loss(loss):
    if loss == "mse":
        ...
        raise ValueError(
            "Please provide a valid entry for loss. Possible choices are: mse, smoothL1"
        )
