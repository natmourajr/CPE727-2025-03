...
def build_dataloaders(batch_size: int = 64) -> Dict[str, DataLoader]:
    transform_train = T.Compose([
        T.Resize((128, 128)),
        T.ToTensor(),
        # normalização opcional possivel medir mean/std depois
    ])
...
def main():
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # Salva melhor modelo com base no F1 de validação
        if val_metrics["f1"] > best_val_f1:
            ...
            print(f"  Novo melhor modelo salvo em {best_ckpt_path} (val_f1={best_val_f1:.4f})")
