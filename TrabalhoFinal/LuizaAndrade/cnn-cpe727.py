# 1D CNN Training with K-Fold (for time series)
import os
import sys
...
class CNN1D_Model(nn.Module):
    def __init__(self, in_channels, sequence_length, num_classes,
                 dropout=0.4, lr=1e-3, weight_decay=1e-5,
        ...
        # Dynamically calculates the encoder output size
        with torch.no_grad():
...
# ============================================================
# 3) RESHAPE FOR 1D CNN (batch, channels, sequence_length)
# ============================================================
# 1D CNN expects format: (batch, in_channels, sequence_length)
# where in_channels are the features (5 columns) and sequence_length is time
...
# Check reshape
expected_features = sequence_length * in_channels
if features_per_window != expected_features:
    ...
    sequence_length = features_per_window // in_channels

# Reshape: (num_samples, total_features) -> (num_samples, in_channels, sequence_length)
# Data comes as [feat1_t1, feat1_t2, ..., feat2_t1, feat2_t2, ...]
# Need to reorganize to [channels, time]
X_data_reshaped = X_data.reshape(num_samples, in_channels, sequence_length)

print(f"Original shape: {X_data.shape}")
print(f"Shape for 1D CNN: {X_data_reshaped.shape}")
...
save_dir_base = f"./results/cnn1d/{timestamp}"
...
plt.title('F1 Score per Fold - Cross-Validation (1D CNN)')
plt.xlabel('Fold')
plt.ylabel('F1 Score')
plt.legend()
...
plt.title("Average Confusion Matrix with Standard Deviation (1D CNN)")
...
sns.heatmap(mean_normalized_cm, annot=normalized_cm_labels_simple, fmt='', 
            ...
            yticklabels=[f'Class {i}' for i in range(n_classes)],
            annot_kws={'size': 14})
plt.title("Normalized Confusion Matrix - Average (1D CNN)", fontsize=14, pad=15)
...
sns.heatmap(mean_normalized_cm, annot=normalized_cm_labels_compact, fmt='', 
            ...
            yticklabels=[f'Class {i}' for i in range(n_classes)],
            annot_kws={'size': 14})
plt.title("Normalized Confusion Matrix - Average ± Standard Deviation (1D CNN)", fontsize=14, pad=15)
plt.xlabel("Predicted", fontsize=14)
plt.ylabel("True", fontsize=14)
plt.tight_layout()
...
with open(results_file, 'w') as f:
    f.write("=== CROSS-VALIDATION RESULTS (1D CNN) ===\n\n")
    f.write(f"Number of Folds: {n_splits}\n")
    f.write(f"Architecture: 1D CNN for Time Series\n")
    f.write(f"  - Input channels: {in_channels}\n")
    f.write(f"  - Sequence length: {sequence_length}\n")
    f.write(f"  - Conv layers: 32 → 64 → 128\n")
    f.write(f"  - Kernel sizes: 7, 5, 3\n")
    f.write(f"  - Pooling: MaxPool1d(2) after each conv\n\n")
