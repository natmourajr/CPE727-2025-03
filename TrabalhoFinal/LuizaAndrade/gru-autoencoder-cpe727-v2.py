...
class GRU_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        return hidden_state[-1]  # Returns the last hidden state

class GRU_Autoencoder_Supervised(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, output_dim,
                 dropout=0.001, lr=1e-4, weight_decay=1e-5,
                 num_epochs=100, class_weights=None):
        super(GRU_Autoencoder_Supervised, self).__init__()
        
        # Encoder (feature extractor)
        self.encoder = GRU_Encoder(input_dim, hidden_dim, num_layers, dropout)
...
print(f"Shape for GRU Autoencoder: {X_data_reshaped.shape}")
...
save_dir_base = f"./results/gru_autoencoder/{timestamp}"
...
for fold, (train_index, val_index) in enumerate(kf.split(X_data_reshaped)):
    ...
    model = GRU_Autoencoder_Supervised(
        input_dim=input_dim,
        hidden_dim=64,
        num_layers=2,
...
plt.title('F1 Score per Fold - Cross-Validation (GRU Autoencoder)')
plt.xlabel('Fold')
plt.ylabel('F1 Score')
plt.legend()
...
plt.title("Average Confusion Matrix with Standard Deviation (GRU Autoencoder)")
...
sns.heatmap(mean_normalized_cm, annot=normalized_cm_labels_simple, fmt='', 
            ...
            yticklabels=[f'Class {i}' for i in range(n_classes)],
            annot_kws={'size': 14})
plt.title("Normalized Confusion Matrix - Average (GRU Autoencoder)", fontsize=14, pad=15)
...
sns.heatmap(mean_normalized_cm, annot=normalized_cm_labels_compact, fmt='', 
            ...
            yticklabels=[f'Class {i}' for i in range(n_classes)],
            annot_kws={'size': 14})
plt.title("Normalized Confusion Matrix - Average ± Standard Deviation (GRU Autoencoder)", fontsize=14, pad=15)
plt.xlabel("Predicted", fontsize=14)
plt.ylabel("True", fontsize=14)
plt.tight_layout()
...
# ============================================================
# RETRAIN BEST FOLD AND SAVE MODEL FOR t-SNE
# ============================================================
print(f"\n🔄 Retraining the best fold ({best_fold_index + 1}) to save the model...")

# Recreate train/val loaders for best fold
...
# Calculate class weights for best fold
...
final_model = GRU_Autoencoder_Supervised(
    input_dim=input_dim,
    hidden_dim=64,
    num_layers=2,
    output_dim=n_classes,
...
# Add get_bottleneck method if it doesn't exist
if not hasattr(final_model, 'get_bottleneck'):
    def get_bottleneck(self, x):
        return self.encoder(x)
    final_model.get_bottleneck = lambda x: final_model.encoder(x)

final_model.fit(train_loader_best, val_loader_best, device)

# Save the model
model_save_path = os.path.join(save_dir_base, "best_gru_autoencoder.pth")
torch.save(final_model.state_dict(), model_save_path)
print(f"✅ Best fold model saved at: {model_save_path}")
...
# Limit sample if dataset is too large
MAX_SAMPLES = 5000
...
plt.title('Bottleneck t-SNE - GRU Autoencoder', fontsize=16, pad=20)
...
tsne_path = os.path.join(save_dir_base, "tsne_gru_bottleneck.png")
plt.savefig(tsne_path, dpi=300, bbox_inches='tight')
plt.close()
print(f"t-SNE saved at: {tsne_path}")
...
print(f"t-SNE coordinates saved at: {csv_path}")
...
with open(results_file, 'w') as f:
    f.write("=== CROSS-VALIDATION RESULTS (GRU AUTOENCODER) ===\n\n")
    f.write(f"Number of Folds: {n_splits}\n\n")
    f.write(f"Architecture: Supervised GRU Autoencoder\n")
    f.write(f"  Encoder (GRU):\n")
    f.write(f"    - Input dim (features): {input_dim}\n")
    f.write(f"    - Hidden dim: 64\n")
    f.write(f"    - Num layers: 2\n")
    f.write(f"    - Dropout: 0.001 (between GRU layers)\n")
    f.write(f"    - Output: last hidden state (64,)\n")
    f.write(f"  Classifier (FC):\n")
    f.write(f"    - Dropout(0.001)\n")
    f.write(f"    - Linear: 64 → 32\n")
    f.write(f"    - ReLU\n")
    f.write(f"    - Dropout(0.001)\n")
    f.write(f"    - Linear: 32 → {n_classes}\n")
    f.write(f"  Input:\n")
    ...
    f.write(f"  - Faster training than LSTM\n")
    f.write(f"  - Performance generally similar to LSTM\n\n")
