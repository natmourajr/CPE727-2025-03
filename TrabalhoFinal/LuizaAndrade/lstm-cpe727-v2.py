# LSTM Training with K-Fold (without early stopping)
import os
import sys
...
# ============================================================
# 1) LSTM_MODEL (without early stopping)
# ============================================================
class LSTM_Model(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, output_dim,
        ...
        # Adjust FC dimension based on bidirectionality
        ...
        # Scheduler to reduce LR when not improving
    ...
    def fit(self, train_loader, val_loader, device):
        # Clear lists for new training cycle
        self.lossList = []
        self.valLossList = []
...
# ============================================================
# 3) RESHAPE FOR LSTM (3D: batch, sequence, features)
# ============================================================
...
# Reshape: (num_samples, total_features) -> (num_samples, sequence_length, input_dim)
X_data_reshaped = X_data.reshape(num_samples, sequence_length, input_dim)

print(f"Original shape: {X_data.shape}")
print(f"Shape for LSTM: {X_data_reshaped.shape}")
...
save_dir_base = f"./results/lstm/{timestamp}"
os.makedirs(save_dir_base, exist_ok=True)

# Convert to PyTorch tensors (maintain 3D format)
...
plt.title('F1 Score per Fold - Cross-Validation (LSTM)')
plt.xlabel('Fold')
plt.ylabel('F1 Score')
plt.legend()
...
plt.title("Average Confusion Matrix with Standard Deviation (LSTM)")
...
sns.heatmap(mean_normalized_cm, annot=normalized_cm_labels_simple, fmt='', 
            ...
            yticklabels=[f'Class {i}' for i in range(n_classes)],
            annot_kws={'size': 14})
plt.title("Normalized Confusion Matrix - Average (LSTM)", fontsize=14, pad=15)
...
sns.heatmap(mean_normalized_cm, annot=normalized_cm_labels_compact, fmt='', 
            ...
            yticklabels=[f'Class {i}' for i in range(n_classes)],
            annot_kws={'size': 14})
plt.title("Normalized Confusion Matrix - Average ± Standard Deviation (LSTM)", fontsize=14, pad=15)
plt.xlabel("Predicted", fontsize=14)
plt.ylabel("True", fontsize=14)
plt.tight_layout()
...
with open(results_file, 'w') as f:
    f.write("=== CROSS-VALIDATION RESULTS (LSTM) ===\n\n")
    f.write(f"Number of Folds: {n_splits}\n")
    f.write(f"Architecture: LSTM\n")
