...
# Models definition:
model = DeepNeuralNetwork(in_features=next(iter(train_loader))[0].shape[1], num_l1=128, num_l2=64)
...
with open(os.path.join(os.path.dirname(__file__), 'history', f"{datetime.now().strftime('%Y_%m_%d_%H_%M')}_dnn_model_history.json"), "w", encoding="utf-8") as f:
...
plot_confusion_matrix(cm, labels=["Normal", "Violado"], save_path=os.path.join(os.path.dirname(__file__), 'history', f"{datetime.now().strftime('%Y_%m_%d_%H_%M')}_dnn_model_cm.png"))