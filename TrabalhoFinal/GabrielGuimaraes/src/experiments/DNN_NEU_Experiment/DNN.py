...
from src.models.DNN_NEU.model import DNN_NEU
...
def main():
    ...
    save_dir = os.path.join("src", "Results", "DNN_NEU")
    ...
    for fold_idx, (train_local_idx, val_local_idx) in enumerate(
        ...
        best_model_fold = DNN_NEU().to(device)
        ...
        # Guarda globalmente se for o melhor fold
    ...
    # -------- 4) Incertezas (média, desvio, IC) sobre os folds --------
    ...
    # CSV com estatísticas de incerteza
    df_unc = pd.DataFrame(
        {
    ...
    plot_loss_and_f1(
        ...
        title_prefix=f"DNN - Fold {best_fold} - "
    )

    print("\n Avaliando o melhor fold no conjunto de TESTE (holdout externo)...")
    best_model = DNN_NEU().to(device)
    ...
    plt.title("Matriz de Confusão - TESTE (Holdout externo) - DNN")
    plt.ylabel("Real")
    plt.xlabel("Predito")
    plt.tight_layout()
