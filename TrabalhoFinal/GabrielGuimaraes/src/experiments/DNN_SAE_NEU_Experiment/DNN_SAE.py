...
from src.models.DNN_SAE_NEU.model import DNN_SAE_NEU
...
def train_one_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for imgs, labels in tqdm(dataloader, desc="Treinando", leave=False):
        ...
        # NÃO faz flatten aqui – o modelo já faz o flatten + encoder interno
        optimizer.zero_grad()
        outputs = model(imgs)
...
# ---------- MAIN EXPERIMENTO (K-Fold + Holdout externo) ----------
...
def main():
    ...
    save_dir = os.path.join("src", "Results", "DNN_SAE_NEU")
    ...
    # Caminho do encoder treinado pelo SAE
    ...
    for fold_idx, (train_local_idx, val_local_idx) in enumerate(
        ...
        model = DNN_SAE_NEU(latent_dim=32).to(device)
        ...
        for epoch in range(max_epochs):
            ...
            # Early stopping baseado em F1 de validação
        ...
        # Reavalia o melhor modelo do fold na validação
        best_model_fold = DNN_SAE_NEU().to(device)
    ...
    df_folds.to_csv(os.path.join(save_dir, "metrics_kfold_folds_DNN_SAE.csv"), index=False)
    ...
    df_unc.to_csv(os.path.join(save_dir, "metrics_kfold_uncertainty_DNN_SAE.csv"), index=False)
    ...
    plot_loss_and_f1(
        ...
        title_prefix=f"DNN+SAE - Fold {best_fold} - "
    )

    print("\n Avaliando o melhor fold no conjunto de TESTE (holdout externo)...")
    best_model = DNN_SAE_NEU(latent_dim=32).to(device)
    ...
    plt.title("Matriz de Confusão - TESTE (Holdout externo) - DNN+SAE")
    plt.ylabel("Real")
    plt.xlabel("Predito")
    plt.tight_layout()
    ...
    df_test.to_csv(os.path.join(save_dir, "metrics_test_holdout_DNN_SAE.csv"), index=False)
