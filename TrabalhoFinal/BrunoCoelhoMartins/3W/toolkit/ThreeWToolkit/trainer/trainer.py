...
from ..models.gru import GRUConfig, GRU
...
class TrainerConfig(ModelTrainerConfig):
    ...
    seed: int
    learning_rate: float
    config_model: MLPConfig | SklearnModelsConfig | CNNConfig | LSTMConfig | GRUConfig
...
class ModelTrainer(BaseStep):
    ...
    def _get_model(
        ...
        match config_model:
            ...
            case CNNConfig():
                return CNN(config_model).to(self.device)
            case LSTMConfig():
                return LSTM(config_model).to(self.device)
            case GRUConfig():
                return GRU(config_model).to(self.device)
            case _:
    ...
    # --- Convert X ---
        if hasattr(x, "values"):                   # pandas DataFrame / Series
            x_np = x.values
        elif isinstance(x, (np.ndarray, np.memmap)):  # numpy array / memmap
            x_np = x
        elif torch.is_tensor(x):                   # already tensor
            X_tensor = x.float()
            x_np = None
        else:
            x_np = np.asarray(x)
        ...
        if y is not None:
            if hasattr(y, "values"):
                y_np = y.values
            elif isinstance(y, (np.ndarray, np.memmap)):
                y_np = y
            elif torch.is_tensor(y):
                y_tensor = y.long() if y.dtype == torch.int64 else y.float()
                y_np = None
            else:
                y_np = np.asarray(y)

            if y_np is not None:
                # For classification, toolkit expects labels as LONG tensors
                y_tensor = torch.tensor(
                    y_np, 
                    dtype=torch.long if y_np.dtype in [np.int32, np.int64] else torch.float32
                )

            dataset = TensorDataset(X_tensor, y_tensor)

        else:
            # If no labels (not typical here)
            y_tensor = torch.empty(len(X_tensor))
            dataset = TensorDataset(X_tensor, y_tensor)
        ...
        **kwargs,
        ...
        Returns:
            dict[str, list[Any]] | None: Training history dictionary for PyTorch
                ...
                scikit-learn models.
        """
        if isinstance(self.model, (MLP, LSTM, GRU)):
