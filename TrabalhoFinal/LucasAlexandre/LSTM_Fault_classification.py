...
class LSTMFaultClassifier(nn.Module):
    def __init__(self, input_size=20000, hidden_size=128, num_layers=2, num_classes=len(class_list), dropout=0.2):
        super().__init__()
    ...
    def forward(self, x):
        # x está no formato correto: [batch, seq_len=20, input_size=20000]
        out, (h, c) = self.lstm(x)

        # usar somente o último hidden state
        h_final = h[-1]  # [batch, hidden_size]
...
def main():
    ...
    model = LSTMFaultClassifier(input_size=20000, hidden_size=128, num_layers=2, num_classes=len(class_list)).to(device)
...
plt.legend()
plt.tight_layout()
plt.savefig('LSTM_training_curves.png', dpi=200)
plt.show()
...
print(df_metrics)
df_metrics.to_csv("LSTM_metrics_per_class.csv", index=False)
