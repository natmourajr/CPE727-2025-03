...
def main():
    ...
    # Descobre C e L automaticamente
    _, C, L = X_train.shape
    input_dim = C * L

    print(f"\n[INFO] DNN input_dim = {input_dim}  (C={C}, L={L})")

    model = DNN(input_dim=input_dim, output_dim=len(class_list)).to(device)
...
plt.legend()
plt.tight_layout()
plt.savefig('DNN_training_curves.png', dpi=200)
plt.show()
...
plt.savefig('DNN_confusion_matrix.png', dpi=200)
plt.show()
...
plt.ylim(45, 101)   # mesmo estilo do seu gráfico de referência
plt.grid(axis='y', linestyle='--', alpha=0.3)
plt.legend()
plt.tight_layout()
plt.savefig("DNN_metrics_per_class.png", dpi=200)
plt.show()
...
plt.savefig("DNN_metrics_table_aligned.png", dpi=200)
plt.show()
...
print(df_metrics)
df_metrics.to_csv("DNN_metrics_per_class.csv", index=False)
