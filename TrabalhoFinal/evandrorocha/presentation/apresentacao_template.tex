...
\begin{frame}{Agenda}
  \begin{enumerate}
      \item \hyperref[sec:intro]{Introdução}
      \item \hyperref[sec:problem]{Problema Abordado}
      \item \hyperref[sec:method]{Método Proposto}
      \item \hyperref[sec:results]{Resultados Obtidos}
        \begin{itemize}
            \item \hyperref[sec:results]{Comparação de Desempenho}
            \item \hyperref[sec:interp]{Interpretabilidade (Grad-CAM)}
        \end{itemize}
      \item \hyperref[sec:concl]{Conclusões e Recomendação}
  \end{enumerate}
\end{frame}
...
\begin{frame}{Contexto}
    \begin{columns}
        ...
        \begin{itemize}
            \item 10 milhões de casos/ano (OMS, 2022) [9]
...
\begin{frame}{Motivação}
    \begin{block}{Por que Deep Learning?}
        \begin{itemize}
            ...
            \item Potencial para auxiliar diagnóstico médico [1][3]
...
\begin{frame}{Pipeline de Processamento (CNNs Pré-treinadas)}
    \begin{enumerate}
        ...
        \item \textbf{Transfer Learning [4]}
        \begin{itemize}
...
\begin{frame}{Modelos Avaliados}
    \begin{table}
        ...
        \begin{tabular}{lccc}
            ...
            \midrule
            ResNet-50 [5] & 25.6M & ImageNet & Skip connections, 50 camadas \\
            DenseNet-121 [6] & 8.0M & ImageNet & Dense connections \\
            EfficientNet-B0 [7] & 5.3M & ImageNet & Compound scaling \\
            \bottomrule
        \end{tabular}
    \end{table}
...
\begin{frame}{Comparação Completa dos Modelos}
    \begin{table}
        ...
        \begin{tabular}{lccccc}
            \toprule
            \textbf{Figura de Mérito} & \textbf{CNN Baseline} & \textbf{ResNet50} & \textbf{DenseNet121} & \textbf{EfficientNet-B0} & \textbf{Melhor} \\
            \midrule
...
\begin{frame}{Evolução do Treinamento (AUC-ROC)}
    \begin{columns}
        \column{0.6\textwidth}
        \includegraphics[width=\textwidth]{imgs/auc_per_epoch_comparison.png}
        
        \column{0.4\textwidth}
        \footnotesize
        \textbf{Interpretação:}
        \begin{itemize}
            \item \textbf{Eixo Y:} Área sob a Curva ROC (Capacidade discriminativa).
            \item \textbf{ResNet-50:} Liderança consistente ($>0.96$).
            \item \textbf{DenseNet-121 (Nota):}
            \begin{itemize}
                \item AUC continuou subindo até a Época 23.
                \item O treinamento parou (Early Stop) porque o \textbf{F1-Score} (prioridade de equilíbrio) parou de melhorar na Época 13.
            \end{itemize}
        \end{itemize}
    \end{columns}
\end{frame}
...
\begin{frame}{Metodologia de Comparação Visual}
    ...
    \begin{block}{O que é Grad-CAM?}
        Técnica que destaca os pixels da imagem que mais influenciaram a decisão da rede neural (Gradient-weighted Class Activation Mapping) [8].
    \end{block}
...
\begin{frame}{Conclusões e Recomendação}
    \begin{block}{Recomendação Principal: \textbf{ResNet-50}}
        \begin{itemize}
            \item \textbf{Maior Sensibilidade (90\%):} Melhor modelo para \textbf{triagem}, minimizando Falsos Negativos (crítico em saúde).
            \item \textbf{Maior AUC-ROC (96\%):} Melhor capacidade geral de separação entre classes.
        \end{itemize}
    \end{block}
    ...
    \begin{columns}
        \column{0.5\textwidth}
        \textbf{Alternativas (Trade-offs):}
        \begin{itemize}
            \item \textbf{EfficientNet:} Máxima Especificidade (94\%). Ideal para confirmação / reduzir falsos alarmes.
            \item \textbf{DenseNet:} Aprendizado mais rápido, mas instável no final.
        \end{itemize}
        
        \column{0.5\textwidth}
        \textbf{Contribuição Técnica:}
        \begin{itemize}
            \item \textbf{Transfer Learning:} Provou reduzir drasticamente o ruído visual (visto no Grad-CAM) comparado à CNN do zero.
            \item \textbf{Generalização:} Modelos pré-treinados evitaram o overfitting severo, comum em datasets pequenos.
        \end{itemize}
    \end{columns}
\end{frame}
...
\begin{frame}{Referências}
    ...
    \begin{thebibliography}{99}
        \bibitem{lakhani2017}
        Lakhani, P., \& Sundaram, B. (2017). Deep learning at chest radiography: automated classification of pulmonary tuberculosis by using convolutional neural networks. \textit{Radiology}, 284(2), 574-582. [1]
        
        \bibitem{rajaraman2018}
        Rajaraman, S., et al. (2018). Pre-trained convolutional neural networks as feature extractors toward improved malaria parasite detection in thin blood smear images. \textit{PeerJ}, 6, e4568. [2]
        
        \bibitem{hwang2016}
        Hwang, S., et al. (2016). A novel approach for tuberculosis screening based on deep convolutional neural networks. \textit{Medical Imaging 2016: Computer-Aided Diagnosis}. [3]
        
        \bibitem{pan2010}
        Pan, S. J., \& Yang, Q. (2010). A survey on transfer learning. \textit{IEEE Transactions on Knowledge and Data Engineering}, 22(10), 1345-1359. [4]
        
        \bibitem{he2016}
        He, K., et al. (2016). Deep residual learning for image recognition. \textit{CvPR}. [5]
        
        \bibitem{huang2017}
        Huang, G., et al. (2017). Densely connected convolutional networks. \textit{CVPR}. [6]
        
        \bibitem{tan2019}
        Tan, M., \& Le, Q. (2019). EfficientNet: Rethinking model scaling for CNNs. \textit{ICML}. [7]
        
        \bibitem{selvaraju2017}
        Selvaraju, R. R., et al. (2017). Grad-CAM: Visual explanations from deep networks via gradient-based localization. \textit{ICCV}. [8]
        
        \bibitem{who2022}
        World Health Organization. (2022). \textit{Global tuberculosis report 2022}. WHO. [9]
    \end{thebibliography}
\end{frame}
