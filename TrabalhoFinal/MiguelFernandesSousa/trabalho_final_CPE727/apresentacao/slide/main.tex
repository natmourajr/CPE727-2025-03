...
\begin{frame}{Tokenização (AG\_NEWS)}
  \small
  \begin{itemize}
  \item Word-level, vocabulário de 10.000 termos (+ \texttt{<PAD>=0}, \texttt{<UNK>=1})
  \item Sequência fixa de 200 tokens: padding/truncamento no fim
  \item Comprimento médio \textasciitilde40 tokens (mín. 12, máx. 183) \(\rightarrow\) \textasciitilde80\% padding
  \item Preserva ordem das palavras; adequado para LSTM capturar dependências sequenciais
  \end{itemize}
\end{frame}
\section{Introdução}
...
\begin{frame}{AG\_NEWS}
  ...
  \item TF-IDF com 10.000 features
  \item Remoção de stop words
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Representação TF-IDF -- Vetores de Entrada}
  ...
  \begin{itemize}
  \item Cada slot = uma palavra do vocabulário com posição fixa
  \item Valores = scores TF-IDF são floats entre 0.0 e $\sim$1.0
  \item Representação esparsa$\sim$98\% zeros 
  \end{itemize}
\end{frame}
...
\begin{frame}{Mapa de Calor -- Termos Mais Frequentes}
  \centering
  \includegraphics[width=0.50\textwidth]{../../eda/outputs/ag_news/top_terms_heatmap.png}
\end{frame}

\begin{frame}{Esparsidade da Matriz TF-IDF}
  \centering
...
\begin{frame}{Modelos Implementados}
  ...
  \item \textbf{Naive Bayes} (3 variantes):
    ...
    \item MultinomialNB: distribuição multinomial (contagem/frequência)
    \end{itemize}
  ...
  \begin{itemize}
  \item \textbf{Regressão Logística}: norma L2, parâmetro C
    \begin{itemize}
...
\begin{frame}{Comparação: Naive Bayes}
  ...
  \textbf{Observação:} Multinomial $>$ Bernoulli $>$ Gaussiano em ambos datasets
\end{frame}
...
\begin{frame}{Gap Generativo vs. Discriminativo}
  ...
  \begin{tabular}{lcc}
  \hline
  \textbf{Dataset} & \textbf{Gap} & \textbf{Possível motivo} \\
  \hline
  Fashion MNIST & \textbf{22.29\%} & Pixels muito correlacionados \\
  AG\_NEWS & \textbf{1.68\%} & TF-IDF gera vetor esparso \\
  \hline
  \end{tabular}
  \end{table}
  ...
  \begin{itemize}
  \item A modalide dos dados pode ter influenciado.
  \item Features densas e correlacionadas como imagens violam suposição de independência do Naive Bayes
  \item Features esparsas e menos correlacionadas são mais adequadas para modelos generativos justamente por supor a independência
  \end{itemize}
\end{frame}
...
\begin{frame}{Problema no Paralelismo da Validação Cruzada}
  \textbf{Problema:} Random Forest no AG\_NEWS consumiu \textbf{96 GB RAM}
