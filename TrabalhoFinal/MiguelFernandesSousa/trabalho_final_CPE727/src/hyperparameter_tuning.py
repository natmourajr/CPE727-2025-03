"""
Fase 1: Hyperparameter Tuning usando Grid Search CV

Busca exaustiva de hiperparâmetros usando validação cruzada (5-fold CV)
com rastreamento completo via MLflow.
"""
...
from src.hyperparameters import get_param_grid, get_total_combinations
...
def get_model_instance(model_name: str):
    ...
    models = {
        ...
        "random_forest": RandomForest(n_jobs=-1, random_state=42),
    }
...
def run_grid_search_cv(
    ...
    mlflow.set_experiment("fashion-mnist-hyperparameter-tuning")

    # Obter modelo e grid
    model = get_model_instance(model_name)
    param_grid = get_param_grid(model_name)
    total_combinations = get_total_combinations(model_name)
    ...
    with mlflow.start_run(run_name=f"{model_name}_gridsearch") as parent_run:
        ...
        # Log CV statistics (mean ± std)
        mlflow.log_metrics(
            {
                "best_cv_accuracy": best_score,
                "best_cv_accuracy_std": grid_search.cv_results_["std_test_accuracy"][best_index],
                ...
                "cv_coefficient_of_variation": (
                    grid_search.cv_results_["std_test_accuracy"][best_index] / best_score
        ...
        try:
            ...
            # Get all CV scores for best config
            ...
            logger.info(f"  Saved CV distribution plot")
        ...
        for rank, idx in enumerate(top_indices, 1):
            ...
            with mlflow.start_run(run_name=f"{model_name}_rank{rank}", nested=True) as nested_run:
                ...
                mlflow.log_metrics(
                    {
                        ...
                        "cv_recall_macro_mean": grid_search.cv_results_["mean_test_recall_macro"][
                            idx
                        ],
