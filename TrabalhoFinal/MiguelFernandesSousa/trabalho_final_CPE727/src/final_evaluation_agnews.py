...
def load_best_params_agnews(model_name: str) -> Dict[str, Any]:
    """
...
def get_model_instance(model_name: str, params: Dict[str, Any]):
    ...
    if model_name in ["gmm", "logistic_softmax", "logistic_ovr", "random_forest"]:
        params["verbose"] = 0
        params["random_state"] = 42

    # Random Forest: always use n_jobs=1 to avoid memory issues
    # (no parallelism needed in final evaluation with single model)
    if model_name == "random_forest" and "n_jobs" not in params:
        params["n_jobs"] = 1

    return models_map[model_name](**params)


def evaluate_final_model_agnews(
    model_name: str,
    X_train_full: np.ndarray,
    y_train_full: np.ndarray,
    X_test: np.ndarray,
    y_test: np.ndarray,
    best_params_agnews: Optional[Dict[str, Any]] = None,
    ...
    Args:
        ...
        X_test: Test set
        y_test: Labels test
        best_params_agnews: Melhores parâmetros (se None, carrega do arquivo)
    ...
    mlflow.set_experiment("final-evaluation-agnews")

    # Carregar melhores parâmetros se não fornecidos
    if best_params_agnews is None:
        best_params_agnews = load_best_params_agnews(model_name)

    logger.info(f"Using parameters: {best_params_agnews}")
    ...
    with mlflow.start_run(run_name=f"{model_name}_final") as run:
        ...
        model = get_model_instance(model_name, best_params_agnews)

        # Log params
        mlflow.log_params({"model_name": model_name, **best_params_agnews})
        mlflow.log_params(
            {
        ...
        tracker._log_confusion_matrix(
            y_test, y_test_pred, f"{model_name}_final", class_names=AG_NEWS_CLASS_NAMES
        )
        tracker._log_classification_report(
            y_test, y_test_pred, f"{model_name}_final", class_names=AG_NEWS_CLASS_NAMES
        )
        ...
        results = {
            ...
            "best_params_agnews": best_params_agnews,
            "metrics": metrics,
...
def evaluate_all_models_agnews(
    X_train_full: np.ndarray,
    y_train_full: np.ndarray,
    X_test: np.ndarray,
    y_test: np.ndarray,
    ...
    with open(results_path, "w") as f:
        ...
        serializable = {
            k: {
                ...
                "best_params_agnews": v["best_params_agnews"],
                "metrics": v["metrics"],
                "run_id": v["run_id"],
            }
