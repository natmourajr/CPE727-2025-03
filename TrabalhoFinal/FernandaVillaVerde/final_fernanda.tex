...
\title[Predição Probabilística de Mudança de Estado]{Predição Probabilística de Mudança de Estado em Compressores Industriais com RNNs e VAEs}
...
\noindent \textbf{Objetivos do trabalho:}
    ...
    \begin{itemize}
     \item Aprender uma \textbf{modelagem probabilística do tempo até a
        ...
        condicionadas ao histórico observado, utilizando RNNs e VAEs;
        \end{itemize}
...
\midrule
\textbf{LSTM RAdam}  & \textbf{0.010} & \textbf{0.001 $\pm$ 0.000} & \textbf{0.000} & \textbf{1.000} & \textbf{0.270} \\
BiLSTM RAdam & 0.023 & 0.002 $\pm$ 0.000 & 0.023 & 0.946 & 0.270 \\
...
\begin{itemize}
      \item Curva de NELBO bastante oscilatória, refletindo o
        ...
        \item A melhor época (\textbf{epoch 126}) atinge o menor NELBO entre as
...
\begin{frame}{Discussão dos resultados — Mudança de estado}
  \begin{itemize}
    \item \textbf{Qualidade probabilística}:
      \begin{itemize}
        \item O \textbf{LSTM RAdam} atinge o \textbf{menor NELBO} entre os modelos,
              indicando melhor ajuste probabilístico para a tarefa de
              mudança de estado;
        \item Baixos valores de NLL e MSE micro confirmam alta precisão
              na modelagem de eventos de transição raros.
      \end{itemize}

    \item \textbf{Calibração das incertezas}:
      \begin{itemize}
        \item O \textbf{LSTM RAdam} apresenta \textbf{cobertura próxima de 1.0},
              evidenciando excelente calibração probabilística;
        \item Width90 constante sugere intervalos preditivos estáveis e
              comparáveis entre arquiteturas.
      \end{itemize}

    \item \textbf{Impacto arquitetural}:
      \begin{itemize}
        \item Arquiteturas unidirecionais mostraram-se mais adequadas ao
              caráter \textbf{causal} do problema de mudança de estado;
        ...
        \item Otimizadores adaptativos (RAdam, warmup) estabilizam o treinamento
              e facilitam a captura de eventos raros, enquanto arquiteturas
              excessivamente complexas tendem a diluir esses sinais.
      \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Conclusões — Mudança de estado}
  \begin{itemize}
    \item A predição de mudanças de estado é um problema
          fortemente desbalanceado e altamente sensível à
          calibração probabilística.

    \item Arquiteturas recorrentes probabilísticas são essenciais:
      \begin{itemize}
        \item Capturam dependências temporais relevantes ao longo da janela;
        \item Modelam explicitamente incertezas associadas à transição de estado.
      \end{itemize}

    \item O \textbf{LSTM RAdam} é o modelo mais adequado neste estudo:
      \begin{itemize}
        \item Melhor NELBO global entre as arquiteturas avaliadas;
        \item Excelente calibração (Cov90 $\approx$ 1.0) com largura
              de intervalo controlada;
        \item Melhor alinhamento com a natureza causal da tarefa.
      \end{itemize}
    ...
    \item Melhorias futuras:
      \begin{itemize}
        \item Utilizar modelos do tipo \textit{Mixture of Gaussians} para capturar
              o próximo estado operacional;
        \item Explorar técnicas de \textit{oversampling} para eventos raros
              (em uma base de 269.800 registros, apenas 572 representam mudanças
              de estado).
      \end{itemize}
  \end{itemize}
\end{frame}
