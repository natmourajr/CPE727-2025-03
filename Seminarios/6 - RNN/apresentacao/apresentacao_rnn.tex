...
% Infos da capa
% ======================
\title[Recorrent Neural Networks - RNNs]{Recorrent Neural Networks - RNNs\\
Arquiteturas, Treinamento e Aplicações}
\author{Fernanda Villa Verde, Leonardo Britto, Luiza Helena e Rodrigo Petrus}
\institute{Utilização de RNNs em séries temporais e processamento de linguagem natural \\
...
\begin{frame}{Graves \& Schmidhuber (2005) - BiLSTM — Bidirectional LSTM (cont.) \cite{graves2005}}
\footnotesize
...
% ======================
\begin{frame}{Arquiteturas Bidirecionais: Acoplada vs. Desacoplada \cite{serdyuk2018}}
\footnotesize
...
\vspace{0.35cm}
\textbf{2) BiRNN Desacoplada (Teacher--Student)}
\begin{itemize}
  ...
  \begin{table}[h]
    ...
    \resizebox{\textwidth}{!}{%
      \begin{tabular}{@{}p{2.2cm}p{3.5cm}p{4cm}p{3.5cm}@{}}
        ...
        $\displaystyle z_t = [\,h_t^{\rightarrow} ; h_t^{\leftarrow}\,]$ &
        \begin{itemize}
          \item Simples e eficiente.
          \item Mantém toda a informação temporal.
          \item Fácil de integrar com MLPs/CNNs.
        \end{itemize}
        &
        \begin{itemize}
        ...
        $\displaystyle z_t = g_t \odot h^{\rightarrow}_t
          + (1-g_t)\odot h^{\leftarrow}_t$,
        onde $\displaystyle g_t = \sigma\!\big(W_g [h^{\rightarrow}_t;h^{\leftarrow}_t]\big)$
        &
        \begin{itemize}
          \item Adapta a contribuição de cada direção por timestep.
          \item Robusto a ruído e desbalanceamento temporal.
          \item Oferece interpretabilidade (causal/anticausal).
        \end{itemize}
        &
        \begin{itemize}
          \item Séries ruidosas (sensores, texto clínico).
          \item Diagnóstico temporal.
        ...
        $\displaystyle
          u_t = \mathrm{GRU}_\text{fuser}
          ([h^{\rightarrow}_t;h^{\leftarrow}_t], u_{t-1})$
        &
        \begin{itemize}
...
\begin{frame}{Gal \& Ghahramani (2016) - Variational Dropout (cont.) \cite{gal2016}}
  ...
  \[
    h_t = f\big(W_{hh}\,(d_h \odot h_{t-1}) + W_{xh}\,(d_x \odot x_t)\big)
  \]
  ...
  \begin{aligned}
    ...
    &d_h, d_x &: \text{máscara de dropout, } d_h, d_x \sim \text{Bernoulli}(p) \\
...
\medskip
\textbf{3) Arquiteturas bidirecionais}
\begin{itemize}
  ...
  \item \textbf{Transformers não substituem RNNs}: estendem a ideia com \textbf{multihead attention}.
\end{itemize}
...
\begin{frame}{Caso de Uso em Séries Temporais - Metodologia de Treinamento e Avaliação}
  ...
  \begin{itemize}
    ...
    \item Early stopping com paciência de 20 épocas para Predição (com 300 épocas no treinamento) e 50 épocas para Reconstrução (com 500 épocas no treinamento).
  \end{itemize}
...
\]
 \vspace{0.2cm}
  \small
  \textbf{Softplus}: função de ativação suave que aproxima a ReLU, 
  evitando regiões mortas e garantindo derivada contínua. 
  Controla-se a suavidade via o parâmetro $\beta$: valores maiores tornam
  a curva mais próxima da ReLU; menores tornam a função mais suave.
\vspace{0.25cm}
\centering
\includegraphics[width=0.45\linewidth]{/home/ferna/CPE727-2025-03/Seminarios/6 - RNN/apresentacao/figs/softplus.png}
...
\begin{frame}{Resultados - MSE micro na Predição}
\centering
\footnotesize
...
& \textbf{Média} 
& \textbf{Desvio padrão} 
& \textbf{Tempo treinamento(min)} 
& \textbf{Melhor época} 
& \textbf{Total de épocas} \\
\midrule
LSTM                         & 0.6795 & {$\pm\,0.3659$} & 1.17 & 15 & 35 \\
GRU                          & 0.5921 & {$\pm\,0.3298$} & 0.88 & 33 & 53 \\
BiLSTM                       & 0.6639 & {$\pm\,0.4266$} & 3.01 & 24 & 44 \\
BiLSTM coupled               & 0.6613 & {$\pm\,0.3704$} & 2.44 & 14 & 34 \\
BiLSTM coupled gate          & 0.6699 & {$\pm\,0.3597$} & 1.98 & 15 & 35 \\
BiLSTM coupled GRU Fuser     & 0.7415 & {$\pm\,0.3805$} & 2.77 & 12 & 32 \\
BiGRU                        & 0.5610 & {$\pm\,0.3509$} & 4.45 & 69 & 89 \\
BiGRU coupled                & 0.5683 & {$\pm\,0.3482$} & 4.80 & 76 & 96 \\
BiGRU coupled gate           & 0.6053 & {$\pm\,0.3852$} & 2.38 & 31 & 51 \\
BiGRU coupled GRU Fuser      & 0.6868 & {$\pm\,0.3208$} & 2.75 & 13 & 33 \\
\bottomrule
\end{tabular}
}
...
\raggedright
\begin{itemize}
  \item[\(\star\)] \textbf{Regra dos Dois Desvios Padrão:} 
  Valores dentro de \(\mu \pm 2\sigma\) correspondem aproximadamente ao intervalo de confiança de 95\%.


  \item[\(\star\)] \textbf{Treinamento:} 
  300 épocas com early stopping de 20 épocas.

  
  \item[\(\star\)] \textbf{Teste de Wilcoxon:} 
  Não foi possível realizar o teste estatístico de Wilcoxon entre os melhores modelos de predição e reconstrução,
  pois há forte desbalanceamento entre as classes.
\end{itemize}
}
...
\begin{frame}{Resultados - MSE micro na Reconstrução}
\centering
\footnotesize
...
& \textbf{Total de épocas} \\
\midrule
LSTM                         & 0.3925 & {$\pm\,0.0443$} & 4.64  & 53  & 103 \\
GRU                          & 0.2914 & {$\pm\,0.0432$} & 3.80  & 70  & 120 \\
BiLSTM                       & 0.1372 & {$\pm\,0.0400$} & 17.73 & 140 & 190 \\
BiLSTM gate                  & 0.1333 & {$\pm\,0.0358$} & 22.14 & 183 & 233 \\
BiLSTM GRU Fuser             & 0.1532 & {$\pm\,0.0380$} & 15.03 & 60  & 110 \\
BiLSTM coupled               & 0.1440 & {$\pm\,0.0334$} & 14.58 & 112 & 162 \\
BiLSTM coupled gate          & 0.1433 & {$\pm\,0.0424$} & 12.64 & 83  & 133 \\
BiLSTM coupled GRU Fuser     & 0.1390 & {$\pm\,0.0433$} & 18.86 & 88  & 138 \\
BiGRU                        & 0.1339 & {$\pm\,0.0355$} & 21.85 & 207 & 257 \\
BiGRU gate                   & 0.1613 & {$\pm\,0.0276$} & 8.36  & 38  & 88  \\
BiGRU GRU Fuser              & 0.1234 & {$\pm\,0.0409$} & 45.05 & 268 & 318 \\
BiGRU coupled                & 0.1414 & {$\pm\,0.0383$} & 13.91 & 99  & 149 \\
BiGRU coupled gate           & 0.1405 & {$\pm\,0.0379$} & 11.57 & 81  & 131 \\
BiGRU coupled GRU Fuser      & 0.1463 & {$\pm\,0.0388$} & 14.63 & 67  & 117 \\
\bottomrule
\end{tabular}
}
...
\begin{itemize}
  ...
  \item[\(\star\)] \textbf{Treinamento:} 
  500 épocas com early stopping de 50 épocas.
...
% ================= SLIDE 5 =================
\begin{frame}{Cao de Uso em Processamento de Linguagem Natural: Hierarchical Attention-based Recurrent Neural Network - HARNN (2019) \cite{huang2019_hierarchical_multilabel_text_classification}}
  \small
...
% ================= SLIDE 4 =================
\begin{frame}{Caso de Uso em Processamento de Linguagem Natural: Hierarchical Attention-based Recurrent Neural Network (HARNN) (2019) \cite{huang2019_hierarchical_multilabel_text_classification}}
  \scriptsize

  % ===== ITEM 1 =====
  
    \textbf{1) Documentation Representing Layer (DRL)}
      \begin{itemize}
         \scriptsize
  ...
  % ===== ITEM 2 (AGORA EM DUAS COLUNAS) =====
   \textbf{2) Hierarchical Attention-based Recurrent Layer (HAM)}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.55\textwidth}
       \scriptsize
      \begin{itemize}
        \item \textbf{TCA - Text--Category Attention}: destaca as palavras mais relevantes para cada categoria do nível,
              comparando as palavras do texto com o vetor (embedding) da própria categoria
              e montando um ``resumo por categoria''.
        \item \textbf{CPM - Category Prediction Module}: usa esses resumos para prever as categorias do nível.
        \item \textbf{CDM - Category Dependency Module}: passa sinais do nível de cima para o de baixo, ajudando a manter
    ...
    \begin{column}{0.35\textwidth}
      \centering
      \includegraphics[width=\linewidth]{/home/ferna/CPE727-2025-03/Seminarios/6 - RNN/apresentacao/figs/HAM.png}
    \end{column}
  \end{columns}
  ...
  % ===== ITEM 3 =====
   ...
   \textbf{3) Hybrid Predicting Layer (HPL)}
      \begin{itemize}
         \scriptsize
...
% ================= SLIDE 17 =================
\begin{frame}{Resultados HARNN -- Dados de teste}
  \scriptsize
  \begin{columns}[T,onlytextwidth]
